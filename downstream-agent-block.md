# Downstream Notes Repo Agent Block

`sync_to_notes_repo.sh` extracts the managed block below and merges it into the
root `AGENT.md` file of a destination notes repository. It replaces only this
managed block on later syncs. Keep destination-specific research, code, data,
and workflow instructions outside the block.
